/*
 * Stub implementations for arm32 (armeabi-v7a) platform.
 * The old sh2_dynarec.c JIT has Clang 17 incompatibilities
 * (implicit int, undeclared clear_cache, changed function signatures).
...
/* SH2Interface_struct for the old dynarec - registered but not used on arm32 */
SH2Interface_struct SH2Dynarec = {
   0x2000,
   "SH2 Dynamic Recompiler (arm32 stub)",
   stub_Init,
   stub_DeInit,
   stub_Reset,
   stub_Exec,
