	...
	: QMainWindow( parent )
{
   ...
   search.clear();
	...
	// 锟絏锟絕锟絙锟絅锟紼锟紹锟絎锟紽锟絙锟絞锟斤拷锟届惉
	mStackedWidget = new QStackedWidget(this);
	setCentralWidget(mStackedWidget);

	// FileSearchWidget锟斤拷锟届惉
...
std::string versionToScalar(const std::string& version) {
	...
	if (parts.size() > 0) {
		result << std::setw(3) << std::setfill('0') << std::setw(3) << parts[0]; // Major僶乕僕儑儞乮1寘乯
	}
	if (parts.size() > 1) {
		result << std::setw(3) << std::setfill('0') << parts[1]; // Minor僶乕僕儑儞乮3寘乯
	}
	if (parts.size() > 2) {
		result << std::setw(3) << std::setfill('0') << parts[2]; // Patch僶乕僕儑儞乮3寘乯
	}
